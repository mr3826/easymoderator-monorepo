...
CI_E2E_ANR_FLAKE=every device flow failed once on the API 34 emulator (Mobile CI 36224789542). A "Pixel Launcher isn't responding" dialog covered the login screen for the whole run, while the app itself was displayed in about 1.5 s for each flow. On emulators only, run-maestro.js now sets hide_error_dialogs=1 for the run (the previous value is restored afterwards) and closes system dialogs before each flow. A physical phone's settings are never touched.
